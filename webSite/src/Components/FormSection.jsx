  ...
  return (
      ...
      <div className="forms-buttons flex flex-wrap md:flex-nowrap ">
        ...
        {forms.map(form => (
          <div>
            {/* form buttpn */}
           <button key={form.id} onClick={() => setActiveFormId(form.id)} className='flex justify-between  items-center px-10 py-2 mt-3 hover:bg-CardBlue focus:bg-CardBlue rounded-lg'>
             
             <div className='text-white text-center' >Form</div>

               {/* delete button */}
              <button onClick={() => deleteForm(form.id)} className='flex py-3 px-3 ml-14  mr-0 hover:bg-Purple  rounded-lg trash'>
                <FaTrash className='' />
               </button>
