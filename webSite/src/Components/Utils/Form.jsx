    ...
    <>
      <div className="">
          ...
          <div className="answer-question  ">
            {form.slice(0, step + 1).map((step, index) => (
              <div key={index} className=" w-full">
                {/* //question */}
                <div className="min-w-[20%] max-w-[60%] w-fit text-white py-3 px-5 mt-6 ml-2 bg-primary rounded-br-3xl rounded-tl-3xl rounded-tr-3xl  break-words ">
                  {step.question ? step.question : step.text}
                </div>
                {/* //answer */}
                {step.answer != "" && (
                  <div className=" min-w-[20%] max-w-[60%] text-white w-fit py-3 px-5 mt-2 ml-2 rounded-br-3xl rounded-tl-3xl rounded-tr-3xl  break-words border-1 ">
                    {step.answer}
                  </div>
                )}
              </div>
              ...
              {form[step] && form[step].type === "choice" ? (
                form[step].options.map((option, index) => (
                  <button
                    ...
                    type="submit"
                    className="bg-Purple text-white py-3 px-6  rounded-lg m-2"
                  >
                    {option}
                  </button>
                ))
        ...
        <div className="navigation-bar flex gap-5 border-t border-white p-2 ">
          ...
          <button
            onClick={handleNext}
            className="px-5 py-3 rounded-lg bg-gradient-to-tr from-Purple to-Blue text-white"
          >
            {" "}
            +{" "}
          </button>
